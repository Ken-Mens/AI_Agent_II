...
from lib.files import retrieve_image_from_resources, open_file, ensure_output_dir_exists
import base64
import os
...
guardrail_agent = Agent(
    ...
    instructions="""
    Check if the image that the user has submitted is a valid floorplan and that the user's design preference input is actually relevant to interior design.
    The user must not ask for anything illegal, offensive, or not safe for work.
    """,
    tools=[],
...
async def run_agent(design_style: str, floorplan_image_path: str):

    ensure_output_dir_exists()

    image = retrieve_image_from_resources(floorplan_image_path)
    ...
    try:
        ...
        for item in result.new_items:
            if (
                ...
                full_output_path = os.path.join("output", file_name)
